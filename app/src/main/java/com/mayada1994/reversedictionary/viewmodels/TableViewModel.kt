package com.mayada1994.reversedictionary.viewmodels
...
class TableViewModel(application: Application) : AndroidViewModel(application) {
    ...
    fun getReverseDictionary(zipfWords: List<ZipfWord>) : List<ZipfWord>{
        return zipfWords.map { it.apply { it.word = word.reversed() } }.sortedBy { it.word }.map { it.apply { it.word = word.reversed() } }
    }
